...
const Board = forwardRef<any, BoardProps>(
  (
    ...
    const [homePositions, setHomePositions] = useState<{
      [key: string]: { homeX: number; homeY: number };
    }>({
    ...
    const doAnimation = (
      ...
      if (targetIndex !== 100) {
        const checkerCoordsStart = checkerPos.find(
          (item) => item.index === checker.props.index
        );
        const endSpike = spikePositions.find(
          (item) => item.index === targetIndex
        );
        const checkerCoordsEnd = calculateAnimationPositionHelper(
          false,
          endSpike!
        );
        handleAnimation(
          checkerCoordsStart!.x,
          checkerCoordsStart!.y,
          checkerCoordsEnd[0],
          checkerCoordsEnd[1]
        );
      } else {
    ...
    const pushChecker = (
      ...
      const oppColor =
        currentPlayer === PLAYER_COLORS.WHITE
          ? PLAYER_COLORS.BLACK
          : PLAYER_COLORS.WHITE;
      if (checker) {
        if (targetIndex === 0 || targetIndex === 25) {
          ...
          if (
            spikes[targetIndex].checkers[0]?.props.color === oppColor &&
            spikes[targetIndex].checkers.length === 1
          ) {
            const oppChecker = spikes[targetIndex].checkers.pop()!;
            const prisonIndex =
              oppChecker.props.color === PLAYER_COLORS.WHITE ? 0 : 25;
            lastMoves.push({
              index: checker.props.index,
              from: sourceIndex,
              to: targetIndex,
    ...
    const undoPosition = async () => {
      ...
      let checker: React.ReactElement<
        ...
        string | React.JSXElementConstructor<any>
      >;
      let oppChecker: React.ReactElement<
        any,
        string | React.JSXElementConstructor<any>
      >;
      if (currentLastMove) {
        if (currentLastMove.to === 100) {
          const indexToRemove = homeChecker.findIndex(
            (checker) => checker.props.index === currentLastMove.index
          );
    ...
    const measureHome = (color: 'white' | 'black') => {
      if (homeRefs.current[color]) {
        homeRefs.current[color]?.measure(
          (x, y, width, height, pageX, pageY) => {
            const homeX =
              pageX + DIMENSIONS.homeWidth - DIMENSIONS.spikeWidth - 5;
    ...
    const calculateAnimationPositionHelper = (
      isStart: boolean,
      spikePos: { index: number; x: number; y: number }
    ) => {
      let x = spikePos.x;
      let y = spikePos.y;
      ...
      if (spikePos.index <= 12) {
        if (spike) {
          if (spike.count === 1) {
            y = spikePos.y + endingBonus + hitBonus;
          } else if (spike.count === 2) {
            y = spikePos.y + checkerWidth + endingBonus;
          } else if (spike.count === 3) {
            y = spikePos.y + 2 * checkerWidth + endingBonus;
          } else if (spike.count === 4) {
            y = spikePos.y + 3 * checkerWidth + endingBonus;
          } else if (spike.count === 5) {
            y = spikePos.y + 3 * checkerWidth + endingBonus;
          } else if (spike.count > 5) {
            y = spikePos.y + 3 * checkerWidth + endingBonus;
          }
    ...
    const calculateAnimationHomeCoords = () => {
      const home =
        currentPlayer === PLAYER_COLORS.WHITE
          ? homePositions['white']
          : homePositions['black'];
      const homeCount = homeChecker.filter(
        (item) => item.props.color === currentPlayer
      ).length;
