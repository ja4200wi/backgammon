...
import { APP_COLORS, COUNTRIES, DIMENSIONS, GAME_TYPE, ICONS } from '../../utils/constants';
...
export function UserProfile({ nickname, extraInfo, country }: { nickname:string,extraInfo: string, country:COUNTRIES}) {
    //add online logic if necessary
  return (
    <View style={{ padding: 16}}>
      <View style={{ flexDirection: 'row', alignItems: 'center' , backgroundColor: APP_COLORS.backgroundColor }}>
...
export function OpenRequest({ nickname, extraInfo, country }: { nickname: string; extraInfo: string; country: COUNTRIES }) {
    return (
      <View style={{ padding: 16 }}>
  ...
  export function Friend({ nickname, country }: { nickname: string; country: COUNTRIES }) {
    return (
      <View style={{ padding: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
  ...
  export function SentRequest({ nickname, country, extraInfo }: { nickname: string; country: COUNTRIES; extraInfo?:string }) {
    return (
      <View style={{ padding: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          {/* Icons (Check and Cancel) aligned to the right */}
          <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
            <Text style={{color:APP_COLORS.standardGrey, fontSize:18}}>{'...waiting'}</Text>
          </View>
...
export function InviteFriend() {
    return (
      <View style={{alignItems:'center',paddingHorizontal: 'auto', marginVertical: 16}}>
        <Button
          radius="sm"
          type="solid"
          ...
          onPress={()=>{console.log('sending invite')}}
        />
      </View>
    );
  }
